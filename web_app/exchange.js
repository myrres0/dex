...
const token_name = 'loveFaceBook';               
const token_symbol = 'LFB';             
...
const exchange_abi =  [
  ...
  },
  {
    "inputs": [
      ...
      {
        "internalType": "uint256",
        "name": "output_reserves",
        "type": "uint256"
      }
    ],
    "name": "countExchangeRate",
    "outputs": [
      {
        "internalType": "uint256",
        "name": "",
...
async function init() {
    ...
    if (poolState['token_liquidity'] === 0
        ...
        for (let i = 1; i < addresses.length; i++) {
            await token_contract.connect(provider.getSigner(addresses[i])).approve(exchange_address, total_supply);

            // Transfer 1000 tokens to each account FOR TESTING PURPOSES ONLY
            //token_contract.connect(provider.getSigner(defaultAccount)).transfer(addresses[i], ethers.utils.parseUnits("1000", 18));
        }
    }
}
...
async function getPoolState() {
    ...
    return {
        token_liquidity: Number(liquidity_tokens) / 10 ** 18,
        eth_liquidity: Number(liquidity_eth) / 10 ** 18,
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    // Algorithm:
    // 1. Make validation checks
    // 2. Calculate the maximum and minimum exchange rates
    // 3. Approve the exchange contract to spend the token
    // 4. Add liquidity to the exchange
    // 5. Set the allowance to 0
    // 6. Log the result

    // Check if the slippage percentage is valid
    if (maxSlippagePct < 0 || maxSlippagePct > 100) {
        throw new Error("Invalid slippage percentage variable");
    }

    // Check if the amount of ETH is valid
    if (amountEth <= 0) {
        throw new Error("Invalid amount of ETH variable");
    }
    var curr_pool = await getPoolState();


    const maxExchangeRate = Math.floor((curr_pool.token_eth_rate * (100 + maxSlippagePct) / 100) * 10**8);
    const minExchangeRate = Math.floor((curr_pool.token_eth_rate * (100 - maxSlippagePct) / 100) * 10**8);

    // Approve the exchange contract to spend the token
    await token_contract.connect(provider.getSigner(defaultAccount))
        .approve(exchange_address, ethers.utils.parseUnits((amountEth * maxExchangeRate).toString(), 18));

    // Add liquidity to the exchange
    const tx = await exchange_contract.connect(provider.getSigner(defaultAccount))
        .addLiquidity(maxExchangeRate, minExchangeRate, { value: ethers.utils.parseUnits(amountEth, 18)});

    // Set the allowance to 0
    await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, 0);

    // Log the result
    console.log("addLiquidity result: ", tx);
}
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    ...
    // 1. Make validation checks
    // 2. Calculate the maximum and minimum exchange rates
    // 3. Remove liquidity from the exchange
    // 4. Log the result
    ...
    const tx = await exchange_contract.connect(provider.getSigner(defaultAccount))
        .removeLiquidity(ethers.utils.parseUnits(amountEth, 18), maxExchangeRate, minExchangeRate);
...
async function removeAllLiquidity(maxSlippagePct) {
    ...
    // Algorithm:
    // 1. Validate the slippage percentage
    // 2. Calculate the maximum and minimum exchange rates
    // 3. Remove all liquidity from the exchange
    // 4. Log the result

    // Validate the slippage percentage
    if (maxSlippagePct < 0 || maxSlippagePct > 100) {
        throw new Error("Invalid slippage percentage variable");
    }
    var curr_pool = await getPoolState();
    ...
    // Remove all liquidity from the exchange
    const tx = await exchange_contract.connect(provider.getSigner(defaultAccount))
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  // Algorithm:
  // 1. Validate the slippage percentage
  // 2. Calculate the minimum exchange rate
  // 3. Approve the exchange contract to spend the token
  ...
  var min_exchange_rate = (100 - Number(maxSlippagePct)) * curr_pool.token_eth_rate * 10**8 / 100;

  // Approve the exchange contract to spend the token
  await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address,BigInt(amountToken)*10n**18n);

  // Swap tokens for ETH
  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(
    ethers.utils.parseUnits(amountToken, 18),
    parseInt(min_exchange_rate)
  );
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  // 1. Validate the slippage percentage
  // 2. Calculate the minimum exchange rate
  // 3. Parse the amount of ETH
  // 4. Swap ETH for tokens
  ...
  var min_exchange_rate = (100 - Number(maxSlippagePct)) * curr_pool.eth_token_rate * 10**8 / 100;
...
const sanityCheck = async function() {
    ...
    start_tokens = start_tokens/10**18;
    ...
    // LP provider rewards implemented
       ...
       //console.log("swap fee: ", swap_fee);
        ...
        var user_tokens1 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
        user_tokens1 = user_tokens1/10**18;
        // console.log("expected_tokens_received: ", expected_tokens_received);
        // console.log("user_tokens1: ", user_tokens1);
        // console.log("start_state.token_liquidity: ", start_state.token_liquidity);
        ...
        user_tokens2 = user_tokens2/10**18;
        //console.log("user_tokens2: ", user_tokens2);
        score += check("Test simple exchange of token to ETH", swap_fee[0], 
          state2.token_liquidity === (state1.token_liquidity + 100) && 
          Math.abs((state1.eth_liquidity - expected_eth_received) - state2.eth_liquidity) < 5 &&
        ...
        var state3 = await getPoolState();
        var user_tokens3 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
        user_tokens3 = user_tokens3/10**18;
        score += check("Test adding liquidity", swap_fee[0], 
          state3.eth_liquidity === (state2.eth_liquidity + 100) &&
          Math.abs(state3.token_liquidity - (state2.token_liquidity + expected_tokens_added)) < 5 &&
          Math.abs(Number(user_tokens3) - (Number(user_tokens2) - expected_tokens_added)) < 5);
        ...
        var state5 = await getPoolState();
        var user_tokens5 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
        user_tokens5 = user_tokens5/10**18;
        score += check("Test removing liquidity", swap_fee[0], 
        ...
        user_tokens6 = user_tokens6/10**18;
        score += check("Test removing all liquidity", swap_fee[0], 
